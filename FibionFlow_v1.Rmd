...
# Specify the relative path to the FibionFlow_v1.Rmd file
here::set_here()
...
fibionfiles <- list.files(path = file.path(here::here(), "Fibion data"), pattern="\\.(CSV|csv)$", recursive=TRUE, full.names=TRUE)
...
# Name the plot
wd <- getwd()
file_name <- paste0(wd, "/Figures/Summaryplot_algorithm_", filename, ".pdf")
...
events <- read_excel(path = file.path(here::here(), "events.xlsx"), sheet=file)
...
# Name the plot
wd <- getwd()
file_name <- paste0(wd, "/Figures/Summaryplot_events_", filename, ".pdf")
