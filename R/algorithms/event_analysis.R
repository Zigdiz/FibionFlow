...
run_event_analysis <- function(filepath, event_file) {
  ...
  # Extract the sheet name from the CSV filename
  sheet_name <- sub("\\.CSV$", "", basename(filepath), ignore.case = TRUE)
  
  # Define a mapping between filenames and sheet names if necessary
  sheet_mapping <- list(
    "ID01" = "School",
  ...
  sheet_names <- excel_sheets(event_file)
  if (!(sheet_name %in% sheet_names) && !(sheet_name %in% names(sheet_mapping))) {
    stop(paste("Sheet", sheet_name, "not found in", event_file))
  }
  
  # Get the correct sheet name from the mapping if necessary
  if (sheet_name %in% names(sheet_mapping)) {
    sheet_name <- sheet_mapping[[sheet_name]]
  }
  
  events <- read_excel(event_file, sheet = sheet_name)
  setDT(events)
  events <- events %>%
    mutate(from = as.POSIXct(from, format = "%m/%d/%y %H:%M"),
           to = as.POSIXct(to, format = "%m/%d/%y %H:%M"))
